...
server.tool(
  ...
  async ({ query, type, maxResults, sortByUpdated }) => {
    ...
    return { content: [{ type: "text" as const, text: tagExternalContent(formatted) }] };
  }
);
...
server.tool(
  ...
  async ({ url, query }) => {
    ...
    return {
      content: [{ type: "text" as const, text: tagExternalContent(formatLlmsInspection(url, llms, { query, maxSections: 12, maxNotesPerSection: 5, maxLinksPerSection: 8 })) }],
    };
  }
);
...
server.tool(
  ...
  async ({ query, maxResults, engine, domain, maxAgeMonths, checkLlmsTxt }) => {
    ...
    return {
      content: [{ type: "text" as const, text: tagExternalContent(formatWebSearchResults(query, enrichedResults, attempts, maxResults, domain)) }],
    };
  }
);
...
server.tool(
  ...
  async ({ url }) => {
    ...
    return { content: [{ type: "text" as const, text: tagExternalContent(formatted || "No links found.") }] };
  }
);
...
// ── SERVER START + SHUTDOWN live in index.ts (bootstrap) ─────────
}
